...
//! only pixel values prove anything. Opt-in via `a3s use browser doctor
...
pub(super) fn check(checks: &mut Vec<Check>, opts: &DoctorOptions) {
    if env::var("AGENT_BROWSER_PROVIDER").is_ok() {
        checks.push(Check::new(
            ...
            CATEGORY,
            Status::Info,
            "Skipped (A3S_USE_BROWSER_PROVIDER is set; would consume cloud quota)",
        ));
        return;
    }
    if env::var("AGENT_BROWSER_CDP").is_ok() {
        checks.push(Check::new(
            ...
            CATEGORY,
            Status::Info,
            "Skipped (A3S_USE_BROWSER_CDP is set; would attach to a real browser)",
        ));
        return;
    }
    ...
    if let Err(e) = send_json(launch_cmd, &session) {
        checks.push(
            ...
            .with_fix("a3s install use/browser   # or check --debug output"),
        );
        return;
    }
...
fn probe_fail(id: &str, message: String) -> Check {
    ...
    if cfg!(target_os = "linux") {
        ...
        check.with_fix("update Chrome to the latest stable (a3s install use/browser)")
    }
}
