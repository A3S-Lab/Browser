    ...
    if let Some(hex) = &key_env {
        if hex.len() == 64 && hex.chars().all(|c| c.is_ascii_hexdigit()) {
            checks.push(Check::new(
                ...
                category,
                Status::Pass,
                "A3S_USE_BROWSER_ENCRYPTION_KEY set (64-char hex)",
            ));
        } else {
            checks.push(
                Check::new(
                    ...
                    category,
                    Status::Fail,
                    "A3S_USE_BROWSER_ENCRYPTION_KEY is not a 64-char hex string",
                )
                .with_fix("export A3S_USE_BROWSER_ENCRYPTION_KEY=$(openssl rand -hex 32)"),
            );
        }
    ...
    if sessions_dir.exists() {
        ...
        if total == 0 {
            ...
            checks.push(
                ...
                .with_fix(format!(
                    "a3s use browser state clean --older-than {}",
                    expire_days
                )),
            );
        } else {
        ...
        if !p.exists() {
            checks.push(
                Check::new(
                    ...
                    Status::Fail,
                    format!(
                        "A3S_USE_BROWSER_ACTION_POLICY points to missing file: {}",
                        policy_path
                    ),
                )
                .with_fix("update or unset A3S_USE_BROWSER_ACTION_POLICY"),
            );
        } else {
