        ...
        loop {
            match rx.try_recv() {
                ...
                Err(broadcast::error::TryRecvError::Lagged(n)) => {
                    eprintln!("[a3s use browser] Warning: CDP event buffer overflowed, {} events dropped. Network requests may be missing from HAR output.", n);
                    continue;
                }
...
async fn handle_pdf(cmd: &Value, state: &DaemonState) -> Result<Value, String> {
    ...
    let save_path = match path {
        Some(p) => p.to_string(),
        None => {
            let dir = crate::product::cache_root().join("pdfs");
...
fn get_har_dir() -> PathBuf {
    crate::product::cache_root().join("har")
}
...
"#,
    ...
    async fn test_handle_har_stop_without_path_uses_default_location() {
        ...
        assert_eq!(har["log"]["creator"]["name"], "a3s-use-browser");
