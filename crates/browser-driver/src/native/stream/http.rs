...
mod tests {
    ...
    #[test]
    fn embedded_dashboard_is_the_real_a3s_bundle() {
        let (_, content_type, index) = serve_embedded_file("/");
        let index = String::from_utf8(index).expect("dashboard index is UTF-8");
        assert_eq!(content_type, "text/html; charset=utf-8");
        assert!(index.contains("A3S Use Browser"));
        assert!(index.contains("/_next/static/"));
        assert!(!index.contains("asset bundle is not installed"));
        assert!(!index.contains("Dashboard not built"));
        assert!(DashboardAssets::iter().count() > 20);
    }
