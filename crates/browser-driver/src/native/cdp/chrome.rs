        ...
        None => find_chrome().ok_or_else(|| {
            ...
            format!(
                "Chrome not found. Checked:\n  \
                 - A3S Use Browser cache: {}\n  \
                 - System Chrome installations\n  \
                 - Puppeteer browser cache\n  \
                 - Playwright browser cache\n\
                 Run `a3s install use/browser` to download Chrome, or use --executable-path.",
                cache_dir.display()
            )
        })?,
    };
    ...
    if cache_dir.exists() {
        let _ = writeln!(
            ...
            "Warning: Chrome cache directory exists ({}) but no Chrome binary found inside. \
             Falling back to system Chrome. Run `a3s install use/browser` to re-download.",
            cache_dir.display()
        );
    }
