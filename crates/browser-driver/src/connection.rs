...
/// Get the A3S-owned base directory for socket and PID files.
pub fn get_socket_dir() -> PathBuf {
    let base = env::var("A3S_USE_BROWSER_SOCKET_DIR")
        .ok()
        ...
        .map(PathBuf::from)
        .unwrap_or_else(product::runtime_root);
...
fn concurrent_daemon_config_error(session: &str) -> String {
    format!(
        "A daemon for session '{}' started concurrently with different daemon configuration. Retry the command so A3S Use Browser can restart it with the requested configuration.",
        session
    )
}
...
mod tests {
    ...
    fn test_get_socket_dir_ignores_empty_socket_dir() {
        ...
        assert!(get_socket_dir().to_string_lossy().contains("a3s"));
    }
    ...
    fn test_get_socket_dir_home_fallback() {
        ...
        assert!(result.to_string_lossy().contains("a3s"));
        assert!(
