...
const CONFIG_BLOCK: &str = "browser";
const CONFIG_ATTRIBUTES: &[&str] = &[
    "headed",
    "json",
    "debug",
    "session",
...
fn read_legacy_json_config_file(path: &Path) -> Option<Config> {
...
fn read_acl_config_file(path: &Path) -> Result<Config, String> {
    let content = fs::read_to_string(path)
        .map_err(|error| format!("failed to read {}: {error}", path.display()))?;
    let document = a3s_acl::parse_acl(&content)
        .map_err(|error| format!("invalid A3S ACL in {}: {error}", path.display()))?;
    let [block] = document.blocks.as_slice() else {
        return Err(format!(
            "{} must contain exactly one browser block",
            path.display()
        ));
    };
    if block.name != CONFIG_BLOCK || !block.labels.is_empty() {
        return Err(format!(
            "{} must contain an unlabeled browser block",
            path.display()
        ));
    }
    ...
    for (name, value) in &block.attributes {
        let normalized = normalize_acl_name(name);
        if !CONFIG_ATTRIBUTES.contains(&normalized.as_str()) {
            return Err(format!(
                "unknown browser ACL attribute '{}' in {}",
                name,
                path.display()
            ));
        }
        if object
            .insert(normalized.clone(), acl_value_to_json(value, path)?)
            .is_some()
        {
            return Err(format!(
                "duplicate browser ACL attribute '{}' in {}",
                normalized,
                path.display()
            ));
        }
    ...
    for nested in &block.blocks {
        if nested.name != "plugin" {
            return Err(format!(
                "unknown browser ACL block '{}' in {}",
                nested.name,
                path.display()
            ));
        }
        plugins.push(parse_acl_plugin(nested, path)?);
    }
    if !plugins.is_empty() {
        object.insert("plugins".to_string(), serde_json::Value::Array(plugins));
    }

    let mut config = serde_json::from_value::<Config>(serde_json::Value::Object(object))
        .map_err(|error| format!("invalid browser ACL values in {}: {error}", path.display()))?;
    config.idle_timeout = parse_idle_timeout_value(
        config.idle_timeout.take(),
        &format!("ACL config {}", path.display()),
    );
    Ok(config)
}

fn parse_acl_plugin(block: &Block, path: &Path) -> Result<serde_json::Value, String> {
    let [name] = block.labels.as_slice() else {
        return Err(format!(
            "plugin blocks in {} require exactly one name label",
            path.display()
        ));
    };
    if !block.blocks.is_empty() {
        return Err(format!(
            "plugin '{}' in {} cannot contain nested blocks",
            name,
            path.display()
        ));
    }
    ...
    for (attribute, value) in &block.attributes {
        let normalized = normalize_acl_name(attribute);
        if !["command", "args", "capabilities", "source"].contains(&normalized.as_str()) {
            return Err(format!(
                "unknown plugin ACL attribute '{}' in {}",
                attribute,
                path.display()
            ));
        }
        object.insert(normalized, acl_value_to_json(value, path)?);
    }
    serde_json::from_value::<PluginConfig>(serde_json::Value::Object(object.clone()))
        .map_err(|error| format!("invalid plugin '{}' in {}: {error}", name, path.display()))?;
    Ok(serde_json::Value::Object(object))
}

fn acl_value_to_json(value: &AclValue, path: &Path) -> Result<serde_json::Value, String> {
    match value {
        AclValue::String(value) => Ok(serde_json::Value::String(value.clone())),
        AclValue::Number(value) => serde_json::Number::from_f64(*value)
            .map(serde_json::Value::Number)
            .ok_or_else(|| format!("non-finite number in {}", path.display())),
        AclValue::Bool(value) => Ok(serde_json::Value::Bool(*value)),
        AclValue::List(values) => values
            .iter()
            .map(|value| acl_value_to_json(value, path))
            .collect::<Result<Vec<_>, _>>()
            .map(serde_json::Value::Array),
        AclValue::Object(values) => {
            let mut object = serde_json::Map::new();
            for (name, value) in values {
                object.insert(normalize_acl_name(name), acl_value_to_json(value, path)?);
            }
            Ok(serde_json::Value::Object(object))
        }
        AclValue::Null => Ok(serde_json::Value::Null),
        AclValue::Call(name, _) => Err(format!(
            "ACL function call '{name}' is not allowed in Browser config {}",
            path.display()
        )),
    }
}

fn normalize_acl_name(name: &str) -> String {
...
fn read_config_file(path: &Path) -> Option<Config> {
    let legacy_json = path.extension().and_then(|value| value.to_str()) == Some("json");
    if legacy_json {
        eprintln!(
            "{} legacy JSON Browser config {}; migrate it to A3S ACL",
            color::warning_indicator(),
            path.display()
        );
        read_legacy_json_config_file(path)
    } else {
        match read_acl_config_file(path) {
...
pub(crate) fn validate_config_file(path: &Path) -> Result<&'static str, String> {
    if path.extension().and_then(|value| value.to_str()) == Some("json") {
        let content = fs::read_to_string(path)
            .map_err(|error| format!("failed to read {}: {error}", path.display()))?;
        serde_json::from_str::<Config>(&content)
            .map_err(|error| format!("invalid legacy JSON in {}: {error}", path.display()))?;
        Ok("valid legacy JSON")
    } else {
        read_acl_config_file(path)?;
        Ok("valid A3S ACL")
    }
}
...
pub fn load_config(args: &[String]) -> Result<Config, String> {
    let explicit = extract_config_path(args)
        .map(|p| ("--config", p))
        .or_else(|| {
            env::var("A3S_USE_BROWSER_CONFIG")
                .ok()
                .map(|p| ("A3S_USE_BROWSER_CONFIG", Some(p)))
        })
        .or_else(|| {
            env::var("AGENT_BROWSER_CONFIG")
                .ok()
                .map(|p| ("AGENT_BROWSER_CONFIG (legacy)", Some(p)))
        });
    ...
    let user_path = crate::product::user_config_path();
    let user_config = if user_path.is_file() {
        read_acl_config_file(&user_path)
            .map_err(|error| format!("failed to load config: {error}"))?
    } else {
        crate::product::legacy_user_config_path()
            .filter(|path| path.is_file())
            .and_then(|path| read_config_file(&path))
            .unwrap_or_default()
    };

    let project_path = crate::product::project_config_path();
    let project_config = if project_path.is_file() {
        Some(
            read_acl_config_file(&project_path)
                .map_err(|error| format!("failed to load config: {error}"))?,
        )
    } else {
        let legacy = crate::product::legacy_project_config_path();
        legacy
            .is_file()
            .then(|| read_config_file(&legacy))
            .flatten()
    };
...
mod tests {
    ...
    #[test]
    fn test_acl_config_deserialize_full() {
        use std::io::Write;
        let dir = tempfile::tempdir().unwrap();
        let config_path = dir.path().join("browser.acl");
  ...
  plugin "vault" {{
    ...
    capabilities = ["credential.read"]
        ...
        let config = read_acl_config_file(&config_path).unwrap();
    ...
    #[test]
    fn test_acl_config_rejects_unknown_attributes_and_calls() {
        use std::io::Write;
        let dir = tempfile::tempdir().unwrap();
        let unknown_path = dir.path().join("unknown.acl");
        let mut file = fs::File::create(&unknown_path).unwrap();
        writeln!(file, "browser {{ mystery = true }}").unwrap();
        assert!(read_acl_config_file(&unknown_path)
            .unwrap_err()
            .contains("unknown browser ACL attribute"));

        let call_path = dir.path().join("call.acl");
        let mut file = fs::File::create(&call_path).unwrap();
        writeln!(file, "browser {{ proxy = env(\"PROXY\") }}").unwrap();
        assert!(read_acl_config_file(&call_path)
            .unwrap_err()
