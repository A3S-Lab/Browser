//! Standard stdio MCP server for exposing A3S Use Browser to MCP clients.
//!
...
fn initialize_result(params: Option<&Value>, config: &McpConfig) -> Value {
    ...
    json!({
        ...
        "serverInfo": {
            "name": crate::product::SERVER_NAME,
            "title": "A3S Use Browser",
...
fn tool_profile_summaries() -> Vec<Value> {
    ...
    .map(|profile| {
        ...
        json!({
            ...
            "usage": format!("a3s use browser mcp --tools {}", profile.name()),
        })
    })
    .collect()
}
    ...
    let mut tools = vec![
        ...
        tool(
            ...
            json!({
                ...
                "headed": { "type": "boolean", "description": "Show the browser window. Explicit true/false overrides A3S_USE_BROWSER_HEADED and config; omit to use those defaults." },
                "webgpu": { "type": "boolean", "description": "Enable WebGPU (SwiftShader software Vulkan on Linux; no GPU required). Explicit true/false overrides A3S_USE_BROWSER_WEBGPU and config; omit to use those defaults." }
            }),
            &[],
        ),
        tool(
...
fn parity_tools() -> Vec<Value> {
    vec![
        ...
        tool(
            TOOL_PLUGIN_ADD,
            "Plugin add",
            "Add a compatibility plugin from npm or GitHub to A3S ACL Browser config.",
            json!({
                ...
                "global": { "type": "boolean", "default": false, "description": "Write ~/.a3s/use/browser/config.acl instead of ./.a3s/use/browser.acl." },
        ...
        tool(
            ...
            json!({ "offline": { "type": "boolean" }, "quick": { "type": "boolean" }, "fix": { "type": "boolean" }, "webgpu": { "type": "boolean", "description": "Also run a live WebGPU render probe (launches a second Chrome)." }, "headed": { "type": "boolean", "description": "Run the WebGPU probe headed to validate the capture path (auto-Xvfb on displayless Linux). Explicit true/false overrides A3S_USE_BROWSER_HEADED/config." }, "debug": { "type": "boolean", "description": "Verbose diagnostics from the probes' scratch daemons." } }),
            &[],
        ),
        tool(
            TOOL_DASHBOARD_START,
        ...
        tool(
            TOOL_UPGRADE,
            "Upgrade",
            "Upgrade A3S Use through its component lifecycle.",
            json!({}),
            &[],
        ),
        tool(
...
fn call_tool(params: Option<&Value>, config: &McpConfig) -> Result<Value, ProtocolError> {
    ...
    if !config.allows(name) {
        return Err(ProtocolError::invalid_params(format!(
            "Tool {} is not enabled by the active MCP tools profile(s): {}. Restart with `a3s use browser mcp --tools all` or add a profile that includes it.",
            name,
            config.profile_names().join(", ")
        )));
    }
...
fn call_tools_profiles(config: &McpConfig) -> Result<Value, ProtocolError> {
    ...
    let text = format!(
        "Active MCP tools profile(s): {}\n\nAvailable profiles:\n{}\n\nRestart the MCP server with `a3s use browser mcp --tools <profile>` or combine profiles with commas, for example `a3s use browser mcp --tools core,network,react`. Use `a3s use browser mcp --tools all` for the full typed CLI parity surface.",
        config.profile_names().join(", "),
        profiles
            .iter()
            .filter_map(|profile| {
    ...
    Ok(json!({
        ...
        "structuredContent": {
            ...
            "profiles": profiles,
            "usage": {
                "default": "a3s use browser mcp",
                "compose": "a3s use browser mcp --tools core,network,react",
                "all": "a3s use browser mcp --tools all",
            }
        },
        "isError": false,
    }))
...
fn call_cli_tool(
    ...
    let run = run_cli(&cli_args, stdin_body, timeout_ms).map_err(|e| {
        ProtocolError::invalid_params(format!("Failed to run a3s use browser: {}", e))
    })?;
...
fn append_timeout_message(stderr: String, timeout_ms: u64) -> String {
    let msg = format!("a3s use browser command timed out after {}ms", timeout_ms);
