...
/// Complete top-level command vocabulary inherited from the locked
/// agent-browser compatibility baseline.
///
/// Keep aliases in this list: parity means existing automation can replace
/// `agent-browser ...` with `a3s use browser ...` without losing a command.
    ...
    match cmd {
            ...
            match sub {
                Some("save") => {
                    let name = rest.get(1).ok_or_else(|| ParseError::MissingArguments {
                        ...
                        usage: "a3s use browser auth save <name> --url <url> --username <user> --password <pass>",
                    })?;
                    ...
                    let url_val = url.ok_or_else(|| ParseError::MissingArguments {
                        context: "auth save".to_string(),
                        usage: "a3s use browser auth save <name> --url <url> --username <user> --password <pass> [--password-stdin]",
                    })?;
                ...
                Some("login") => {
                    const AUTH_LOGIN_USAGE: &str = "a3s use browser auth login <name> [--credential-provider <plugin>] [--item <ref>] [--url <url>] [--username-selector <s>] [--password-selector <s>] [--submit-selector <s>]";
                    let name = rest.get(1).ok_or_else(|| ParseError::MissingArguments {
                ...
                Some("delete") | Some("remove") => {
                    let name = rest.get(1).ok_or_else(|| ParseError::MissingArguments {
                        ...
                        usage: "a3s use browser auth delete <name>",
                    })?;
                ...
                Some("show") => {
                    let name = rest.get(1).ok_or_else(|| ParseError::MissingArguments {
                        ...
                        usage: "a3s use browser auth show <name>",
                    })?;
        ...
        "confirm" => {
            let cid = rest.first().ok_or_else(|| ParseError::MissingArguments {
                ...
                usage: "a3s use browser confirm <confirmation-id>",
            })?;
        ...
        "deny" => {
            let cid = rest.first().ok_or_else(|| ParseError::MissingArguments {
                ...
                usage: "a3s use browser deny <confirmation-id>",
            })?;
...
mod tests {
    ...
    #[test]
    fn locked_upstream_top_level_command_inventory_is_complete() {
        let mut commands = TOP_LEVEL_COMMANDS.to_vec();
